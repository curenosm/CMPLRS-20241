...
test_majority_1 =
  TestCase (assertEqual "majorityElem [3,2,3]" 3 (majorityElem [3, 2, 3]))
...
test_majority_3 =
  TestCase (assertEqual "majorityElem [2,2,2]" 2 (majorityElem [2, 2, 2]))

test_majority_4 =
  TestCase
    (assertEqual
       "majorityElem [2,2,2,1,1,1]"
       2
       (majorityElem [2, 2, 2, 1, 1, 1]))
...
test_subsets_3 =
  TestCase (assertEqual "subsets []" (sort [[]]) (sort (subsets ([] :: [Int]))))

test_subsets_4 =
  TestCase (assertEqual "subsets [4]" (sort [[], [4]]) (sort (subsets [4])))
...
test_coins_1 = TestCase (assertEqual "coins [2,5] 8" True (coins [2, 5] 8))

test_coins_2 =
  TestCase (assertEqual "coins [2,4,6] 21" False (coins [2, 4, 6] 21))

test_coins_3 =
  TestCase
    (assertEqual "coins [1,2,3] 6" True (coins [1, 2, 3] 6) -- Possible with [3,3] or [1,2,3]
     )

test_coins_4 =
  TestCase
    (assertEqual "coins [5,10] 3" False (coins [5, 10] 3) -- Impossible to make 3 with denominations 5 and 10.
     )

test_coins_5 =
  TestCase
    (assertEqual "coins [5,10,20] 35" True (coins [5, 10, 20] 35) -- Possible with [5,10,20]
     )
...
test_isBST_3 =
  TestCase
    (assertEqual "isBST Empty" True (isBST Empty) -- An empty tree should be considered a valid BST.
     )

test_isBST_4 =
  TestCase
    (assertEqual
       "isBST (Node 2 (Node 1 Empty Empty) (Node 3 Empty Empty))"
       True
  ...
  TestCase
    (assertEqual
       ...
       (kthElem
          ...
          4) -- Last element of a 4 node tree.
     )
